...
exports.constraints = constraints => {
  const groupedByName = utils.group('constraint_name', constraints);

  const sorted = Object.values(groupedByName).map(
    utils.sortBy(utils.get('ordinal_position'))
  );

  return sorted.map(constraintChunks => {
    return constraintChunks.reduce((constraint, chunk) => {
      ...
      if (constraint) {
        constraint = {
          ...constraint,
          columns: [...constraint.columns, column_name],
        };
        if (constraint.references) {
          constraint.references = {
            ...constraint.references,
            columns: [...constraint.references.columns, referenced_column_name],
          };
        }
        return constraint;
      } else {
